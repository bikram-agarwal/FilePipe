...
class RunAllScheduledRulesWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        return try {
            val results = executeRulesUseCase(rules, TriggerType.SCHEDULED) { progress ->
                synchronized(progressNotifyLock) {
    ...
    private fun refreshBatchProgressNotification(rules: List<Rule>, progress: RunProgress) {
        if (rules.size > 1) {
            ...
            val builder = NotificationCompat.Builder(appContext, FileOrganizerWorker.CHANNEL_ID)
                .setSmallIcon(R.drawable.ic_notification)
                .setOngoing(true)
            ...
            if (allRulesInBatchFinished) {
                ...
                val fileLabel = progress.currentFileName.ifBlank {
